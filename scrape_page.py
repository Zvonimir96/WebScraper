...
from scraper.database import save_apartments as db_save, get_existing_ids
...
def main():
    ...
    # Save to SQLite (deduplicates by URL automatically)
    total = db_save(apartments)
    
    logger.success(f"Scraped {len(apartments)} apartments (total in DB: {total})")
